...
model = awg.AWG("R",50,"Ni",5,"di",3,"df",2);
...
ni = 5;
no = 40;
s0 = model.li + (ni - (model.Ni-1)/2)*max(model.di,model.wi);
s1 = (no - (model.N-1)/2)*model.d;
...
a0 = atan(sin(t0) / (1 + cos(t0))); % k-vector angle
...
% draw input curve
t = linspace(-1/2,1/2) * (model.Ni + 4) * model.ai;
Cx = r * sin(t);
Cz = r * (1 - cos(t));
...
% local origin
[x0, z0] = dealx(ILocalToWorld * [0;-df] + [s0x;s0z]);
    ...
    % draw light axis
    plot([z0,R],[x0,0],'k','LineWidth',2)
    
    % draw radius
    plot([s0z,r],[s0x,0],'k','LineWidth',2)
